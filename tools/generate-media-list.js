#!/usr/bin/env node
/**
 * Generates media-list.json at build time so the Media page works on Netlify
 * without needing the serverless function to access the large media folder.
 */
const fs = require("fs");
...
const CATALOG_PATH = path.join(PROJECT_ROOT, "assets/data/media-catalog.json");
const OUTPUT_PATH = path.join(PROJECT_ROOT, "assets/data/media-list.json");
...
const walkMedia = (dirPath) => {
  ...
  entries.forEach((entry) => {
    ...
    if (!type) return;
    files.push({ absolutePath: absolute, type, stat: fs.statSync(absolute) });
  });
  return files;
};
...
function generate() {
  if (!fs.existsSync(MEDIA_ROOT)) {
    const data = { source: "media-folders", categories: [], count: 0 };
    fs.writeFileSync(OUTPUT_PATH, JSON.stringify(data, null, 0), "utf-8");
    console.log("media-list: no media folder, wrote empty list");
    return;
  }
  ...
  if (includeUnmapped) {
    rootFolders.forEach((folder) => {
      ...
      ordered.push({ folder, name: folder, slug: slugify(folder) });
    });
  }
  ...
  const data = { source: "media-folders", count, categories };
  fs.writeFileSync(OUTPUT_PATH, JSON.stringify(data, null, 0), "utf-8");
  console.log(`media-list: wrote ${categories.length} categories, ${count} items`);
}
